...
- 卡片只使用 1px、7% 白色透明度的弱边界；四象限和横向层级卡片用它表达可点击/可展开，不使用阴影、渐变或发光。
...
- `#levels` 与 `#false-transformation` 共用 `.insight-carousel / .insight-track / .insight-card` 和 `[data-card-*]` 脚本，不复制两套滚动或展开逻辑。
- 两组轨道独立维护位置和 `01 / 03` 索引；同一组最多展开一张，卡片外部高度固定。
- 图标为项目本地 Lucide 对应节点的静态 SVG 输出，不依赖额外浏览器运行时。
- Levels 和 Transformation 通过 section 背景、卡片背景与卡高 token 区分，不机械复制成六张连续同款卡片。
- 修改后必须检查 1440×900、1024×768、390×844、320×568，确保只有轨道横向滚动，页面本身不溢出。

## 后续落地注意

- 不要改评分模型、题库、登录、数据库或付费能力，只把首页视觉和交互映射到现有页面。
- 当前 Next.js 首页直接读取本静态文件；不要在 React 层复制 Hero、LevelPath 或 FalseTransformation。业务接入只替换既有 DOM 中的数据和链接。
