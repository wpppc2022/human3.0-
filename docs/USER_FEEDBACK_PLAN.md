...
本文件最初用于 1.0 发布前的小样本真实用户反馈。用户现已取消准确性研究和相关招募，因此本文件只保留取消状态与 archive/optional 模板，不是当前执行计划。
...
当前公开测试仍使用 48 题。32 题平衡版是 `draft / unvalidated` 候选，未上线。

## 32 题准确性研究（已取消）

2026-07-05 产品决定取消 32 题认知访谈、48/32 配对、复测、招募和晋级统计。当前不再派发或执行这些活动，也不冻结用于校准的 questionSetHash。

- `h3-a48-v1` 继续作为 `stable` 正式默认。
- `h3-a32-v1` 无限期保持 `draft / unvalidated`，不进入 `calibrating` 或 `stable`。
- 不得宣称 32 题准确性、等价性、角色公平性或复测稳定性已经验证。
- 已有访谈、配对、数据模板和晋级门槛保留为 archive/optional；没有新的用户/总控授权时不得继续。
- 软件单元/API/E2E 测试继续保留，但只属于工程验证，不是产品准确性测验。

归档材料见 `docs/ASSESSMENT_32_RESEARCH_PLAN.md`、`docs/calibration/` 和三个认知访谈文档。它们不得被当作当前执行清单。

## 现有 48 题一般产品反馈模板（Archive / Optional）

以下是早期 1.0 用户体验反馈模板，不属于当前 32 题准确性研究，也没有现行招募任务。只有用户未来单独授权一般 UX 研究时才可使用；不得借此恢复阶段准确性、阈值或 48/32 等效性验证。
