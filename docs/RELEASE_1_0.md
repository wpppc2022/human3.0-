...
- 完成 PDF 标题和手机端菜单修复：PDF 不再使用数字章节序号或小节标题前圆点，小节标题无占位并直接左对齐；正文列表圆点继续保留，横线维持撤销 MUJI 试验后的上一版；首页移动端菜单和共享导航移动端菜单已可打开、进入二级、返回、关闭并锁定/恢复 body 滚动。
- 完成首页返回一致性修复：从测评页、结果页及移动端菜单进入 `/` 或 `/#...` 时使用完整文档加载，确保 Canvas 粒子脚本、关键 DOM、导航和滚动/锚点状态与首次直接打开一致。
- PDF 标题圆点和首页返回一致性修复后再次运行 `pnpm check`，数据校验、63 个单元测试、lint、生产构建和 13 个端到端测试全部通过。
- 首页 `#levels` 与 `#false-transformation` 已改为共享横向卡片系统：三卡、Lucide 线性图标、固定高度详情切换、同组单开、箭头/索引、移动 scroll-snap 和 reduced-motion 均已落地；其他首页 section 与业务链路未改。
- 首页双卡片 section 重构后再次运行 `pnpm check`，数据校验、63 个单元测试、lint、生产构建和 15 个端到端测试全部通过。
- 完成移动 carousel P0 reveal 修复：逐卡 reveal 已提升到 carousel 容器，离屏卡始终 `opacity:1/transform:none`，390px/320px 下一卡可见，两组轨道无纵向滚动；1440 无溢出时箭头是否隐藏仍作为 P2 待用户确认。
- 移动 carousel reveal P0 修复后再次运行 `pnpm check`，数据校验、63 个单元测试、lint、生产构建和 15 个端到端测试全部通过。
- 手机端菜单修复后再次运行 `pnpm check`，数据校验、63 个单元测试、lint、生产构建和 11 个端到端测试全部通过。
...
- 同步范围：首页静态 UI、Canvas/完整加载修复、两组已验收卡片、问卷/进度、真实评分与 API、结果页、分享卡、两页 A4 PDF、移动菜单及对应正式产品测试和文档。
- 最新需求：模板化已暂停并降为末级 backlog；所有模板目录、组件、样式、Debug 路由、模板测试和演示资源均不进入本次发布。
- 数据口径：本次不修改题库、评分算法或结果口径；`lib/report-pdf 2.ts` 旧副本明确排除。
