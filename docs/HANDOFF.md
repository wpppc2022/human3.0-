...
- `tests/e2e/`：端到端测试，覆盖完整测评流程、刷新恢复、无本地结果、无效分享链接、提交 API、分享链接和移动端核心控件。
...
- 端到端测试已覆盖核心流程、无本地结果、无效分享链接和提交 API；尚未覆盖下载 PNG 和更多边界输入。
...
5. 推送远程后观察首次 CI 运行，并继续扩展 PNG 下载和边界输入端到端测试。
