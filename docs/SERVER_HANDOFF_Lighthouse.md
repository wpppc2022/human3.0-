...
- HUMAN domain: `human.wpppc.cn` is reserved for the next release and must be isolated from Orbit.
...
| `human3` | not deployed in this release step | HUMAN 3.0 Next.js service | Use an independent directory, process, port and Caddy site block |
...
- private bind `127.0.0.1:3100` unless an approved port change is recorded;
...
sudo ss -ltnp
sudo caddy validate --config /opt/infrastructure/caddy/Caddyfile
...
- Do not stop, recreate or upgrade `caddy`, `portainer` or Orbit services for a HUMAN release.
